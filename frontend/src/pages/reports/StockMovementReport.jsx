...
// A filtered, chronological list of stock movements -- the same
// underlying data as the History page (Phase 16), but framed as a
// report with grouped totals per movement type shown up top.
export default function StockMovementReport() {
  ...
  useEffect(() => {
    setPage(1);
  }, [productFilter, warehouseFilter, startDate, endDate]);
  ...
  useEffect(() => {
    ...
    loadReport();
  }, [productFilter, warehouseFilter, startDate, endDate, page]);
      ...
      {totalsByType.length > 0 && (
        <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
          {totalsByType.map((row) => {
            const meta = STOCK_TRANSACTION_TYPES[row._id];
            return (
              <div key={row._id} className="rounded-lg bg-white p-4 shadow">
